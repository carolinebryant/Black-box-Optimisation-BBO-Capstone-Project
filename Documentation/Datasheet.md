...
Several transformations were applied during modelling, including log-abs and piecewise scaling, log transforms, standard scaling, and rank transforms depending on the function. The raw .npy files are preserved unchanged; all transforms are applied in-memory and do not overwrite the source data. Candidate points for acquisition were drawn from a mix of strategies depending on the function, including uniform random sampling, Latin Hypercube sampling, targeted perturbations around the best observed point, and dimension-specific sweeps; in some cases, an SVR surrogate was used to shortlist candidates before the main acquisition step. 

**Intended use:** educational demonstration of Bayesian Optimisation under a limited evaluation budget for the Capstone project. 
